...
then their API domains must be added - however, note that some proxies (e.g. pipelock
with TLS interception will interfere with the traffic if the requests are sent
as generic proxy traffic).
