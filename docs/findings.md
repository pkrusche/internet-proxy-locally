...
Iron is now selectable, but its live measurements are pending. The tables below
retain the earlier three-engine batch. A complete Docker run of
`uv run ipl-lab measure` is required before Iron can enter this comparison;
see [lab.md](lab.md#reproducing-the-comparison).
...
| Measured | off: 2026-09-10T16:47:12Z<br>on: 2026-09-10T16:47:17Z | 2026-09-10T16:47:22Z (off only) | off: 2026-09-10T16:47:27Z<br>on: 2026-09-10T16:47:32Z | off: 2026-09-10T16:47:36Z<br>on: 2026-09-10T16:47:41Z |
...
| [dns-private-ipv4](#dns-private-ipv4) | PASS [metadata+private-ip] | PASS [private-ip] (off only) | PASS [metadata+private-ip] | ERROR |
| [dns-private-ipv6](#dns-private-ipv6) | PASS [private-ip] | PASS [private-ip] (off only) | PASS [private-ip] | ERROR |
...
* **Pipelock (TLS off)** — PASS (expectation: allow, 311ms)  
...
* **Squid (TLS off)** — PASS (expectation: allow, 40ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
* **Squid (TLS on)** — PASS (expectation: allow, 30ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
* **Iron (TLS off)** — PASS (expectation: allow, 34ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
* **Iron (TLS on)** — PASS (expectation: allow, 25ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
...
* **Pipelock (TLS on)** — PASS (expectation: allow, 20ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Smokescreen (TLS off)** — PASS (expectation: allow, 29ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Squid (TLS off)** — PASS (expectation: allow, 25ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Squid (TLS on)** — PASS (expectation: allow, 39ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Iron (TLS off)** — PASS (expectation: allow, 27ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
...
* **Squid (TLS off)** — PASS [hostname-not-allowlisted] (expectation: deny, 10ms)  
...
* **Squid (TLS on)** — PASS [hostname-not-allowlisted] (expectation: deny, 8ms)  
...
* **Pipelock (TLS off)** — PASS [metadata+private-ip] (expectation: deny, 176ms)  
...
* **Pipelock (TLS on)** — PASS [metadata+private-ip] (expectation: deny, 14ms)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Smokescreen (TLS off)** — PASS [private-ip] (expectation: deny, 15ms)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Squid (TLS off)** — PASS [metadata+private-ip] (expectation: deny, 14ms)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Squid (TLS on)** — PASS [metadata+private-ip] (expectation: deny, 23ms)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Iron (TLS off)** — ERROR (expectation: deny, 17ms)  
  HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032); HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032); HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032); HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032)
* **Iron (TLS on)** — ERROR (expectation: deny, 21ms)  
  HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway; HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway; HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway; HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway

### dns-private-ipv6

The same, for IPv6 (sslip.io).

* **Pipelock (TLS off)** — PASS [private-ip] (expectation: deny, 152ms)  
...
* **Pipelock (TLS on)** — PASS [private-ip] (expectation: deny, 10ms)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Smokescreen (TLS off)** — PASS [private-ip] (expectation: deny, 11ms)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Squid (TLS off)** — PASS [private-ip] (expectation: deny, 12ms)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Squid (TLS on)** — PASS [private-ip] (expectation: deny, 17ms)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Iron (TLS off)** — ERROR (expectation: deny, 12ms)  
  HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032); HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032); HTTP/1.1 200 Connection Established — inconclusive TLS/HTTP exchange after CONNECT: [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032)
* **Iron (TLS on)** — ERROR (expectation: deny, 16ms)  
  HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway; HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway; HTTP/1.1 200 Connection Established — inconclusive HTTP response after TLS: HTTP/1.1 502 Bad Gateway — bad gateway
...
* **Pipelock (TLS off)** — PASS [private-ip] (expectation: deny, 1598ms)  
...
* **Pipelock (TLS on)** — PASS [private-ip] (expectation: deny, 1584ms)  
  no connection reached the trap. 3/3 names were resolved more than once and so were handed the private address (9 lookups total); of the 3 repeat probes, 3 were denied, 0 carried HTTP traffic, and 0 were inconclusive
...
* **Squid (TLS off)** — PASS [private-ip] (expectation: deny, 1589ms)  
  no connection reached the trap. 3/3 names were resolved more than once and so were handed the private address (6 lookups total); of the 3 repeat probes, 3 were denied, 0 carried HTTP traffic, and 0 were inconclusive
* **Squid (TLS on)** — PASS [private-ip] (expectation: deny, 1613ms)  
  no connection reached the trap. 3/3 names were resolved more than once and so were handed the private address (6 lookups total); of the 3 repeat probes, 3 were denied, 0 carried HTTP traffic, and 0 were inconclusive
* **Iron (TLS off)** — PASS (expectation: deny, 1598ms)  
...
* **Iron (TLS on)** — PASS (expectation: deny, 1605ms)  
  no connection reached the trap. 3/3 names were resolved more than once and so were handed the private address (6 lookups total); of the 3 repeat probes, 0 were denied, 0 carried HTTP traffic, and 3 were inconclusive
...
* **Pipelock (TLS off)** — PASS [private-ip] (expectation: deny, 130ms)  
...
* **Pipelock (TLS on)** — PASS [private-ip] (expectation: deny, 75ms)  
  control public-only.fixture.test established, and both mixed-answer names (public-first and private-first) were denied — every address in the answer set is validated, not only the first one or the routable one
* **Smokescreen (TLS off)** — FAIL (expectation: deny, 64ms)  
...
* **Squid (TLS off)** — PASS [private-ip] (expectation: deny, 66ms)  
  control public-only.fixture.test established, and both mixed-answer names (public-first and private-first) were denied — every address in the answer set is validated, not only the first one or the routable one
* **Squid (TLS on)** — PASS [private-ip] (expectation: deny, 70ms)  
  control public-only.fixture.test established, and both mixed-answer names (public-first and private-first) were denied — every address in the answer set is validated, not only the first one or the routable one
* **Iron (TLS off)** — FAIL (expectation: deny, 70ms)  
  mixed-public-first.fixture.test:443 established — the engine connected although a private address was in the answer set; mixed-private-first.fixture.test:443 established — the engine connected although a private address was in the answer set
* **Iron (TLS on)** — FAIL (expectation: deny, 70ms)  
  mixed-public-first.fixture.test:443 established — the engine connected although a private address was in the answer set; mixed-private-first.fixture.test:443 established — the engine connected although a private address was in the answer set
...
* **Squid (TLS off)** — PASS [ip-literal-destination] (expectation: deny, 38ms)  
...
* **Squid (TLS on)** — PASS [ip-literal-destination] (expectation: deny, 39ms)  
...
* **Pipelock (TLS off)** — PASS [sni-mismatch] (expectation: deny, 40ms)  
...
* **Pipelock (TLS on)** — PASS [sni-mismatch] (expectation: deny, 40ms)  
  mismatched SNI refused: tunnel established but TLS handshake failed (SNI=files.pythonhosted.org): [SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1032) — the matching-SNI control to the same host completed (tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)), so the refusal is the proxy's rather than the origin's
* **Smokescreen (TLS off)** — FAIL [sni-mismatch] (expectation: deny, 32ms)  
...
* **Squid (TLS on)** — FAIL [sni-mismatch] (expectation: deny, 32ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
* **Iron (TLS off)** — FAIL [sni-mismatch] (expectation: deny, 40ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
...
* **Pipelock (TLS on)** — PASS [non-tls-in-tunnel] (expectation: deny, 17ms)  
  tunnel established; connection closed with no response to raw (non-TLS) bytes — consistent with a non-TLS-in-tunnel policy check
* **Smokescreen (TLS off)** — FAIL [non-tls-in-tunnel] (expectation: deny, 26ms)  
...
* **Squid (TLS off)** — FAIL [non-tls-in-tunnel] (expectation: deny, 21ms)  
  raw bytes traversed the tunnel; response: type=alert(21) version=TLS1.2 length=2 -> alert level=fatal(2) description=decode_error(50); type=alert(21) version=TLS1.2 length=2 -> alert level=warning(1) description=close_notify(0)
* **Squid (TLS on)** — PASS [non-tls-in-tunnel] (expectation: deny, 2ms)  
  tunnel established; connection closed with no response to raw (non-TLS) bytes — consistent with a non-TLS-in-tunnel policy check
* **Iron (TLS off)** — FAIL [non-tls-in-tunnel] (expectation: deny, 28ms)  
...
* **Iron (TLS on)** — FAIL [non-tls-in-tunnel] (expectation: deny, 30ms)  
  raw bytes traversed the tunnel; response: not a recognized TLS record; first bytes: 485454502f312e3120333031204d6f766564205065726d616e656e746c790d0a (b'HTTP/1.1 301 Moved Permanently\r\n')
...
* **Pipelock (TLS on)** — PASS (expectation: allow, 30ms)  
  10 concurrent CONNECTs to an allowed host: 10 established, 0 denied/failed
* **Smokescreen (TLS off)** — PASS (expectation: allow, 22ms)  
  10 concurrent CONNECTs to an allowed host: 10 established, 0 denied/failed
