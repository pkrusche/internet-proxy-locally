...
| --- | --- | --- | --- |
| Measured | 2026-09-09T14:17:47Z | 2026-09-09T14:17:53Z | 2026-09-09T14:18:02Z |
...
| Result | 14 pass, 1 record, 2 skip, 2 error | 13 pass, 3 fail, 1 record, 2 skip | 14 pass, 2 fail, 1 record, 2 skip |
| Exit code | 1 | 1 | 1 |
...
* **Pipelock** does not pass `dns-rebinding` (skip), `ptr-allowlist` (skip), `connect-sni-mismatch` (error), `connect-raw-tunnel` (error).
* **Smokescreen** does not pass `dns-rebinding` (skip), `dns-mixed-answers` (fail), `ptr-allowlist` (skip), `connect-sni-mismatch` (fail), `connect-raw-tunnel` (fail).
* **Squid** does not pass `dns-rebinding` (skip), `ptr-allowlist` (skip), `connect-sni-mismatch` (fail), `connect-raw-tunnel` (fail).
...
* [`connect-raw-tunnel`](#connect-raw-tunnel) — Pipelock ERROR; Smokescreen/Squid FAIL [non-tls-in-tunnel]

**Same behavior, different stated reason** on 8 check(s). These are not behavioral differences — the request was refused either way — but they say which rule did the refusing, which is what decides whether a row is evidence of the thing it is named after:
...
| [dns-rebinding](#dns-rebinding) | full | deny | SKIP | SKIP | SKIP |
| [dns-mixed-answers](#dns-mixed-answers) | full | deny | PASS [private-ip] | FAIL | PASS [private-ip] |
| [ptr-allowlist](#ptr-allowlist) | full | deny | SKIP | SKIP | SKIP |
| [connect-sni-mismatch](#connect-sni-mismatch) | full | deny | ERROR | FAIL [sni-mismatch] | FAIL [sni-mismatch] |
| [connect-raw-tunnel](#connect-raw-tunnel) | full | deny | ERROR | FAIL [non-tls-in-tunnel] | FAIL [non-tls-in-tunnel] |
...
* **Pipelock** — PASS (expectation: allow, 308ms)  
  reached pypi.org (HTTP/1.1 200 OK)
* **Smokescreen** — PASS (expectation: allow, 45ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
* **Squid** — PASS (expectation: allow, 28ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
...
* **Squid** — PASS [hostname-not-allowlisted] (expectation: deny, 10ms)  
...
* **Squid** — PASS [hostname-not-allowlisted] (expectation: deny, 2ms)  
  denied: HTTP/1.1 403 Forbidden — 403 Forbidden internet-proxy-locally denied this request: the destination is not in the allowlist.
...
* **Squid** — PASS [ip-literal-destination] (expectation: deny, 2ms)  
...
* **Squid** — PASS [private-ip] (expectation: deny, 5ms)  
...
* **Pipelock** — PASS [hostname-not-allowlisted] (expectation: deny, 5ms)  
...
* **Pipelock** — PASS [metadata+private-ip] (expectation: deny, 165ms, 4 probes)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Smokescreen** — PASS [private-ip] (expectation: deny, 24ms, 4 probes)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
...
* **Smokescreen** — PASS [private-ip] (expectation: deny, 18ms, 3 probes)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Squid** — PASS [private-ip] (expectation: deny, 19ms, 3 probes)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
...
* **Pipelock** — PASS [private-ip] (expectation: deny, 48ms, 3 probes)  
...
* **Squid** — PASS [private-ip] (expectation: deny, 21ms, 3 probes)  
  control public-only.fixture.test established, and both mixed-answer names (public-first and private-first) were denied — every address in the answer set is validated, not only the first one or the routable one
...
* **Pipelock** — SKIP (expectation: deny, 14ms)  
  the local DNS fixture is not observable from here, so the PTR claim for 1.0.0.1 cannot be known to be live. Run `ipl-lab up` and `ipl-lab check`, which wires the fixture's container through automatically
* **Smokescreen** — SKIP (expectation: deny, 14ms)  
  the local DNS fixture is not observable from here, so the PTR claim for 1.0.0.1 cannot be known to be live. Run `ipl-lab up` and `ipl-lab check`, which wires the fixture's container through automatically
* **Squid** — SKIP (expectation: deny, 14ms)  
  the local DNS fixture is not observable from here, so the PTR claim for 1.0.0.1 cannot be known to be live. Run `ipl-lab up` and `ipl-lab check`, which wires the fixture's container through automatically
...
* **Pipelock** — ERROR (expectation: deny, 19ms)  
...
* **Smokescreen** — FAIL [sni-mismatch] (expectation: deny, 37ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
* **Squid** — FAIL [sni-mismatch] (expectation: deny, 29ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
...
* **Squid** — FAIL [non-tls-in-tunnel] (expectation: deny, 23ms)  
  raw bytes traversed the tunnel; response: type=alert(21) version=TLS1.2 length=2 -> alert level=fatal(2) description=decode_error(50); type=alert(21) version=TLS1.2 length=2 -> alert level=warning(1) description=close_notify(0)
...
* **Pipelock** — RECORD (expectation: record, 48ms)  
  10 concurrent CONNECTs to an allowed host: 10 established, 0 denied/failed
* **Smokescreen** — RECORD (expectation: record, 27ms)  
  10 concurrent CONNECTs to an allowed host: 10 established, 0 denied/failed
* **Squid** — RECORD (expectation: record, 24ms)  
  10 concurrent CONNECTs to an allowed host: 10 established, 0 denied/failed
