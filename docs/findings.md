...
| --- | --- | --- | --- |
| Measured | 2026-09-09T14:57:33Z | 2026-09-09T14:57:38Z | 2026-09-09T14:57:44Z |
...
| Result | 16 pass, 1 record, 2 error | 15 pass, 3 fail, 1 record | 15 pass, 2 fail, 1 record, 1 error |
| Exit code | 1 | 1 | 1 |
...
* **Squid** does not pass `dns-rebinding` (error), `connect-sni-mismatch` (fail), `connect-raw-tunnel` (fail).
...
**Different behavior** on 4 check(s) — one engine allowed what another refused:
...
* **Pipelock** — PASS (expectation: allow, 309ms)  
...
* **Pipelock** — PASS (expectation: allow, 34ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Smokescreen** — PASS (expectation: allow, 39ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Squid** — PASS (expectation: allow, 32ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
...
* **Squid** — PASS [hostname-not-allowlisted] (expectation: deny, 14ms)  
...
* **Pipelock** — PASS [metadata+private-ip] (expectation: deny, 166ms, 4 probes)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Smokescreen** — PASS [private-ip] (expectation: deny, 22ms, 4 probes)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
...
* **Pipelock** — PASS [private-ip] (expectation: deny, 122ms, 3 probes)  
...
* **Smokescreen** — PASS [private-ip] (expectation: deny, 18ms, 3 probes)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Squid** — PASS [private-ip] (expectation: deny, 18ms, 3 probes)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
...
* **Pipelock** — PASS [private-ip] (expectation: deny, 1586ms, 6 probes)  
...
* **Smokescreen** — PASS [private-ip] (expectation: deny, 1602ms, 6 probes)  
...
* **Squid** — ERROR (expectation: deny, 1633ms, 6 probes)  
...
* **Pipelock** — PASS [private-ip] (expectation: deny, 24ms, 3 probes)  
...
* **Smokescreen** — FAIL (expectation: deny, 45ms, 3 probes)  
...
* **Smokescreen** — PASS [hostname-not-allowlisted] (expectation: deny, 29ms, 1 probes)  
...
* **Squid** — PASS [ip-literal-destination] (expectation: deny, 28ms, 1 probes)  
...
* **Pipelock** — ERROR (expectation: deny, 20ms)  
...
* **Squid** — FAIL [sni-mismatch] (expectation: deny, 28ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
...
* **Pipelock** — ERROR (expectation: deny, 17ms)  
...
* **Smokescreen** — FAIL [non-tls-in-tunnel] (expectation: deny, 24ms)  
...
* **Squid** — FAIL [non-tls-in-tunnel] (expectation: deny, 24ms)  
  raw bytes traversed the tunnel; response: type=alert(21) version=TLS1.2 length=2 -> alert level=fatal(2) description=decode_error(50); type=alert(21) version=TLS1.2 length=2 -> alert level=warning(1) description=close_notify(0)
...
* **Pipelock** — RECORD (expectation: record, 40ms)  
