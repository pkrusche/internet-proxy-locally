...
| --- | --- | --- | --- |
| Measured | 2026-09-09T19:01:31Z | 2026-09-09T19:01:40Z | 2026-09-09T19:02:01Z |
...
| Result | 18 pass, 1 record | 15 pass, 3 fail, 1 record | 4 pass, 13 fail, 1 record, 1 error |
| Exit code | 0 | 1 | 1 |
...
* **Squid** does not pass `blocked-host-connect` (fail), `direct-ip-connect` (fail), `loopback-ipv4` (fail), `rfc1918-ipv4` (fail), `link-local-ipv4` (fail), `metadata-endpoint` (fail), `loopback-ipv6` (fail), `private-ipv6` (fail), `dns-private-ipv4` (fail), `dns-private-ipv6` (fail), `dns-rebinding` (error), `dns-mixed-answers` (fail), `ptr-allowlist` (fail), `connect-sni-mismatch` (fail).
...
* **Pipelock** — PASS (expectation: allow, 316ms)  
  reached pypi.org (HTTP/1.1 200 OK)
* **Smokescreen** — PASS (expectation: allow, 34ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
* **Squid** — PASS (expectation: allow, 36ms)  
  reached pypi.org (HTTP/1.1 301 Moved Permanently)
...
* **Smokescreen** — PASS (expectation: allow, 71ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
* **Squid** — PASS (expectation: allow, 316ms)  
  tunnel established, TLSv1.3 handshake OK (SNI=pypi.org)
...
* **Squid** — FAIL [unknown] (expectation: deny, 645ms)  
...
* **Squid** — FAIL [unknown] (expectation: deny, 503ms)  
...
* **Squid** — FAIL [unknown] (expectation: deny, 505ms)  
...
* **Squid** — FAIL [unknown] (expectation: deny, 1513ms)  
...
* **Squid** — FAIL [unknown] (expectation: deny, 504ms)  
...
* **Squid** — FAIL [metadata] (expectation: deny, 510ms)  
...
* **Squid** — FAIL [unknown] (expectation: deny, 1008ms)  
...
* **Smokescreen** — PASS [private-ip] (expectation: deny, 12ms, 4 probes)  
  allowlisted hostnames resolving to private/loopback/metadata IPv4 all denied; resolved IPs recorded per attempt
* **Squid** — FAIL (expectation: deny, 2043ms, 4 probes)  
...
* **Smokescreen** — PASS [private-ip] (expectation: deny, 16ms, 3 probes)  
  allowlisted hostnames resolving to private/loopback IPv6 all denied; resolved IPs recorded per attempt
* **Squid** — FAIL (expectation: deny, 1531ms, 3 probes)  
...
* **Smokescreen** — PASS [private-ip] (expectation: deny, 3109ms, 6 probes)  
...
* **Squid** — ERROR (expectation: deny, 4589ms, 6 probes)  
...
* **Pipelock** — PASS [private-ip] (expectation: deny, 555ms, 3 probes)  
...
* **Smokescreen** — FAIL (expectation: deny, 1571ms, 3 probes)  
...
* **Squid** — FAIL (expectation: deny, 1517ms, 3 probes)  
  mixed-public-first.fixture.test:443 established — the engine connected although a private address was in the answer set; mixed-private-first.fixture.test:443 established — the engine connected although a private address was in the answer set
...
* **Pipelock** — PASS [hostname-not-allowlisted] (expectation: deny, 31ms, 1 probes)  
...
* **Smokescreen** — PASS [hostname-not-allowlisted] (expectation: deny, 31ms, 1 probes)  
...
* **Squid** — FAIL (expectation: deny, 537ms, 1 probes)  
...
* **Smokescreen** — FAIL [sni-mismatch] (expectation: deny, 28ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
* **Squid** — FAIL [sni-mismatch] (expectation: deny, 36ms)  
  mismatched SNI accepted: tunnel established, TLSv1.3 handshake OK (SNI=files.pythonhosted.org)
...
* **Pipelock** — PASS [non-tls-in-tunnel] (expectation: deny, 19ms)  
...
* **Smokescreen** — RECORD (expectation: record, 22ms)  
  10 concurrent CONNECTs to an allowed host: 10 established, 0 denied/failed
